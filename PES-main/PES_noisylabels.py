...
from common.tools import AverageMeter, getTime, evaluate, predict_softmax, train,elr_loss
...
def MixMatch_train(epoch, net, optimizer, labeled_trainloader, unlabeled_trainloader, class_weights):
    ...
    with tqdm(total=num_iter,leave=True,desc="MixMatch_train") as pbar:
...
def noisy_refine(model, train_loader, num_layer, refine_times):
    ...
    for epoch in range(refine_times):
        train(model, train_loader, optimizer_adam, ceriation, epoch,0)
        _, test_acc = evaluate(model, test_loader, ceriation,"Epoch " + str(epoch) + " Test Acc:",epoch,0)
...
if __name__=='__main__':
    ...
    ceriation = elr_loss(args.batch_size*len(train_loader))
    ...
    for epoch in range(args.num_epochs):
        if epoch < args.T1:
            train(model, train_loader, optimizer, ceriation, epoch,1)
        else:
            if epoch == args.T1:
        ...
        _, test_acc = evaluate(model, test_loader, ceriation,"Epoch " + str(epoch) + " Test Acc:",epoch,1)
        best_test_acc = test_acc if best_test_acc < test_acc else best_test_acc
        scheduler.step()

    torch.save(model.state_dict(), "PES10Agg.pth")
